...
interface Props {
  isRollMode?: boolean
  student: Person
  onStateChange: (newState: RolllStateType, student: Person) => void
}
export const StudentListTile: React.FC<Props> = ({ isRollMode, student,onStateChange }) => {
  return (
    <S.Container>
      <S.Avatar url={Images.avatar}/>
      <S.Content>
      ...
      {isRollMode && (
        <S.Roll>
          <RollStateSwitcher
            initialState={student.roll === null ? "unmark" : student.roll}
            onStateChange={(next) => onStateChange(next, student)}
          />
        </S.Roll>
      )}
    </S.Container>
